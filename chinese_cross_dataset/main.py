...
def train(args):
    ...
    logger.info("********** Scheme: Loading training data ... **********")

    tokenizer = BertTokenizer.from_pretrained(args.bert_model, do_lower_case=True)
    train_features, train_type_sent_map = read_and_load_examples(args, tokenizer, "train")
    train_start_id = {key:0 for key in train_type_sent_map.keys()}

    # obtain evaluation dataset
    logger.info("********** Scheme: Loading validation data ... **********")
    # from pdb import set_trace; set_trace()
    eval_batch_support, eval_batch_query = get_eval_items(args, "val")
    ...
    for step in trange(0, int(args.max_train_steps), desc="Steps: ", disable=False):
        ...
        # batch_query, batch_support = train_corpus.get_batch_meta(batch_size=args.batch_size)  # (batch_size=32)
        batch_support, batch_query= get_batch_meta_from_features(
            args,
            train_features, 
            train_type_sent_map, 
            train_start_id, 
        ...
        span_loss, type_loss, loss = learner.forward_joint(batch_query, batch_support, progress=progress)
        ...
        if (step % args.eval_every_train_steps == 0 and step > protect_step) or (step == args.max_train_steps - 1):
            ...
            # result_valid, predictions_valid = test(args, learner, eval_batch_support, eval_batch_query, "valid")
            # F1_valid = result_valid["f1"]

            F1_valid = evaluate(args, learner, "val", eval_batch_support, eval_batch_query)
            is_best = False
            
            if F1_valid > F1_valid_best["all"]:
                logger.info("===> Best Valid F1: {}".format(F1_valid))
                logger.info("  Saving model...")
                learner.save_model(args.result_dir, "en", args.max_seq_length, "all")
...
if __name__ == "__main__":
    ...
    # dataset setting 
    parser.add_argument("--data_path", type=str, default="data/chinese_cross_dataset", help="path of datasets")
    parser.add_argument("--types_path", type=str, default="data/chinese_cross_dataset/entity_types_chinese_domain.json", help="path of entities types")
    # parser.add_argument("--N", type=int, default=4, help="index of test dataset, ontonotes: 4, wnut: 3, gum: 2, conll: 1")
    parser.add_argument("--eval_dataset", type=str, default="zh_ccks_medical_shot", help="Test dataset")
